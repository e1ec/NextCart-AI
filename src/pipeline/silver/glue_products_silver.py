...
# ── Cast types ───────────────────────────────────────────────
products_df = (
    products_df.withColumn("product_id", F.col("product_id").cast(IntegerType()))
...
aisles_df = aisles_df.withColumn("aisle_id", F.col("aisle_id").cast(ShortType())).withColumn(
    "aisle", F.trim(F.lower(F.col("aisle")))
)

departments_df = departments_df.withColumn(
    "department_id", F.col("department_id").cast(ShortType())
).withColumn("department", F.trim(F.lower(F.col("department"))))

# ── Drop nulls and duplicates ────────────────────────────────
products_df = products_df.filter(
    F.col("product_id").isNotNull() & F.col("product_name").isNotNull()
).dropDuplicates(["product_id"])

# ── Normalise product name ───────────────────────────────────
products_df = products_df.withColumn(
    "product_name_clean", F.trim(F.regexp_replace(F.lower(F.col("product_name")), r"\s+", " "))
)

# ── Join into denormalised table ─────────────────────────────
enriched_df = (
    products_df.join(aisles_df.select("aisle_id", "aisle"), on="aisle_id", how="left")
...
# ── Write silver ─────────────────────────────────────────────
(enriched_df.write.mode("overwrite").partitionBy("department_id").parquet(f"{SILVER}/"))
